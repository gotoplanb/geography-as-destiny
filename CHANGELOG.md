...
- **Epilogue — closing capstone fixed.** After seven "is still there" clauses (the steppe, the gravity wells, the first island chain, Malacca, the Bosphorus, Suez, Panama), the summarizing sentence read "None of these terrain features will *be* there in any humanly relevant timeframe" — which negates the whole point. Inverted to match the refrain: "None of them is going anywhere in any humanly relevant timeframe."
- **Appendix A §1 — the Goldilocks settlement test revised to multi-variable.** The entry had proposed a pure-elevation SRTM test for the river settlement gradient. Chapter 4 (v5.1) has since run a first-pass literature test and established rainfall/soil as a co-equal controlling variable — the upstream/downstream asymmetry confirmed, but the single-variable "earliest settlement originates in a middle-river zone" claim failing for the Fertile Crescent and the Indus, where farming began in the rain-fed uplands and hilly flanks (Jarmo, Mehrgarh). Section 1 now reports that result and reframes the still-outstanding quantitative test as a three-layer GIS overlay (elevation + rainfall/isohyet + soil), keeping Chapter 4's cross-reference honest and anchoring reference cards 071, 340, and 341.
- **Appendix A §10 — the Neo-Assyrian case resolved as a sharpening, not a falsifier.** The expansion-contraction economic-policy cycle's falsification clause listed "expansion-phase empires adopting extraction policies" as a falsifier, and the Neo-Assyrian Empire — extraction-heavy (tribute, mass deportation, terror) during its expansion, not merely its contraction — appeared to trip it. It does not: the load-bearing claim is causal (extraction erodes the substrate), and Assyria confirms it at maximum amplitude — an extraction-dependent economy that never became self-sustaining and so collapsed totally and permanently (Nineveh, 612 BCE) rather than fragment-and-persisting like Rome into Byzantium or the Chinese dynastic order. The section now adds an Assyria worked case and rewrites the falsification clause to center the causal core and demote the expansion↔markets phase-pairing to a derived tendency shaped by survivorship (the Wald apparatus of Appendix C: the empires that extract during expansion don't endure to enter the sample of "enduring empires" whose policy histories we read). New reference card 343.

## v5.1.4 — 2026-08-31

**Patch: three epilogue additions and two Chapter 11 corrections. No framework or new-evidence changes — the epilogue applies the existing apparatus to the present, and the Chapter 11 fixes correct the honest-accounting register.**

- **Epilogue — India added.** The contemporary-geopolitics section's US–China–Europe–Russia corridor framing had passed over a top-three power. India now gets its own paragraph (Section III), fitting the framework by a *different* mechanism than Europe/Russia: not corridor-position but **insulation**. The Himalayas, ocean on three sides, and the northwestern mountain-desert wall it off; the India–China frontier is the roof of the world, which gates the friction (Doklam 2017, Galwan 2020) below the full collision two nuclear billion-person states on an open frontier would risk — hence no full war since 1962. The one comparatively open frontier, the Punjab plains toward Pakistan, is exactly where the wars were fought (now a nuclear equilibrium). The same walls that protect India box its own land reach in, leaving it consequential at sea — the Indian Ocean pivot, the maritime Silk Road's node — and that insulation is what lets it hold the middle road, keeping strategic autonomy and working with all sides at once. The prose keeps the discipline: insulation *enables* autonomy, it does not determine it (a well-played draw), and India is not low-force in general (1971 Bangladesh, Sri Lanka's IPKF, the Maldives). Offered for challenge, not as a specialist's verdict. New reference card 342; footnote `ep-4`.
- **Epilogue — the 1991 hinge.** The unipolar framing had skipped the hinge it rests on: American unipolarity is not an old condition but a recent and brief one, dating only from the 1991 Soviet collapse — and the same Soviet Union had armed the CCP toward victory in 1949 and built much of China's first heavy industry in the 1950s before the split. The re-emergence of asymmetric multipolarity reads as the return of the normal condition after a roughly thirty-year anomaly, not the loss of a natural order.
- **Epilogue — the China "invisible" overclaim corrected.** The sender-side-encoding passage had implied the post-9/11 narrative made China's rise invisible. It did not: China's rise was discussed throughout and the regional specialists never stopped watching. What the encoding did was *demote* it — the distortion was misallocation, not blindness; the gradient was legible to anyone who looked, but the attention and the money went overwhelmingly elsewhere.
- **Chapter 11 — card count corrected.** "Thousands" of reference cards overstated the archive; it is *hundreds*. Fixed.
- **Chapter 11 — the "speed" accounting made honest.** The compression claim ("took decades, now took days") was tightened to the actual arc: weeks to the concept and a committee-ready draft, then directional confirmation, then the detailed rigor added over months — roughly five months of editing, including listening to the audiobook of the book itself. The honest number, not the dramatic one.
