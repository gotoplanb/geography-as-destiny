...
Each semester covers a different intellectual area and builds the evidence base for specific chapters. The plan projected five. In practice the formal semester structure was kept for two terms and then dropped — not because the reading stopped, but because it no longer needed the scaffolding. From there each major book functioned as its own semester: the same cascade of seminar, deep read, reference harvest, and scaffolding, run per-book instead of per-term. Eleven of the deep-read works carry a full write-up in the literature review, and the reading across both phases produced 344 reference cards. The projection was met and passed; what changed was the bookkeeping, not the method.
